...
class DNSCacheService {
  ...
  public getExpiryTime(domain: string, type?: DNSRecord['type']): number | null {
    const domainCache = this.cache.get(domain);
    if (!domainCache) return null;

    const records = Array.from(domainCache.values())
      .filter(cached => !type || cached.record.type === type);

    if (records.length === 0) return null;

    return Math.min(...records.map(r => r.expiresAt));
  }
